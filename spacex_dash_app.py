...
# Extract unique locations and add "All Places" to the options
launch_site = spacex_df['Launch Site'].unique().tolist()
dropdown_options = [{'label': i, 'value': i} for i in launch_site]
dropdown_options.insert(0, {'label': 'All Sites', 'value': 'ALL'})  # Add "All Places" option
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(filtered_df.loc[filtered_df['class'] == 1 ], values='count', 
        names='Launch Site', 
        title='Total Success Launch By Site')
        return fig
    else:
        # return the outcomes piechart for a selected site
        fig = px.pie(filtered_df.loc[filtered_df['Launch Site'] == entered_site ], values='count', 
        names='class', 
...
def update_scatter_chart(entered_site , payload_range):
    filtered_df = spacex_df.loc[:,['Launch Site','Payload Mass (kg)','class','Booster Version Category']]
